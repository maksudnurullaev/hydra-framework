	...
	private static String getAllAnyHtml(
		...
		if(result.isOk() && result.getObject() instanceof WebApplications){
			...
			if(apps.getApplications() != null){
				...
				content.append("<select id=\"admin.select.applications\" onchange=\"javascript:void(Globals.sendMessage({handler: 'Adm',action: 'getApp', appid:this.value, dest: 'admin.app'})); return false;\">");
