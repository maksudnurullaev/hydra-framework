...
public class CassandraAccessorBean extends ACassandraAccessor {
	public ResultAsListOfColumnOrSuperColumn find(CassandraVirtualPath inPath) {
		...
		case KSP___CF___ID:
			...
			cf = new ColumnParent(inPath.getPathPart(PARTS.P2_CF));
			key = KEY_COLUMNS_DEF;
			predicate = DBUtils.getSlicePredicate(inPath
					.getPathPart(PARTS.P3_KEY), inPath
		...
		case KSP___CF___ID___LINKNAME:
			...
			predicate = DBUtils.getSlicePredicate(inPath
					.getPathPart(PARTS.P4_SUPER), inPath
	...
	public Result update(CassandraVirtualPath inPath,
		...
		try {
			for (Map<String, Map<String, List<Mutation>>> batchMap : generateMutationMap(
					inPath, inBatchMap)) {
				client.batch_mutate(inPath._kspBean.getName(), batchMap,
	 ...
	 * @param inPath
	 * @param inBatchMap
	 * @return {@code List<Map<KeyString, Map<ColumnFamilyString,
	 *         List<Mutation>>>>}
	 */
	private List<Map<String, Map<String, List<Mutation>>>> generateMutationMap(
		...
		getLog().debug(
				String.format(
						"Generate mutation list for: %s, access path type: %s",
						inPath.getPath(), inPath.getPathType()));
		switch (inPath.getPathType()) {
		case KSP___CF:
		case KSP___CF___ID:
		...
		default:
			getLog()
					.error(
							String
									.format(
											"Unknow access path type to create mutation list for: %s, access path type: %s",
	...
	private void generateMutationMap4KspCfIDLinks(CassandraVirtualPath inPath,
		...
		CassandraVirtualPath linksVPath = new CassandraVirtualPath(inPath
				.getDescriptor(), String.format("%s.%s", inPath._kspBean
				.getName(), inPath._cfLinkBean.getName()));
		// create mutation
		List<byte[]> linkIDs = generateMutationMap4KspCf(linksVPath,
				inBatchMap, result);
	...
	private void generateMutation4Links(CassandraVirtualPath inPath,
		...
		for (byte[] bytes : linkIDs) {
			listOfColumns.add(new Column(bytes, DBUtils
					.string2UTF8Bytes(Constants.GetCurrentDateTime()), System
					.currentTimeMillis()));
		}

		// generate super_column
		SuperColumn superColumn = new SuperColumn();
		superColumn.setName(DBUtils.string2UTF8Bytes(inPath._cfLinkBean
				.getName()));
		...
		// set Cf
		mapCfListMutaion
				.put(inPath._kspBean.getLinkTableName(), listOfMutation);
	...
	private List<byte[]> generateMutationMap4KspCf(CassandraVirtualPath inPath,
		...
		List<Mutation> listOfMutation = new ArrayList<Mutation>();
		// Super Col Val
		for (Map.Entry<byte[], Map<byte[], byte[]>> mapIdColsVals : inBatchMap
				.entrySet()) {
			// setup List<Column> object
			List<Column> listOfColumns = new ArrayList<Column>();
			for (Map.Entry<byte[], byte[]> mapColVal : mapIdColsVals.getValue()
					.entrySet()) {
				listOfColumns.add(new Column(mapColVal.getKey(), mapColVal
		...
		getLog().debug(
				"Added super columns count: " + mapKeyMapCfListMutaion.size());
	...
	public Result delete(String inKsp, ColumnPath inCf, String inKey) {
		return delete(inKsp, inCf, inKey, System.currentTimeMillis(),
				ConsistencyLevel.ONE);
	}

	public Result delete(DeletePack inPack){		
		return delete(
				inPack.getKsp(), inPack.getCf(), inPack.getKey(), 
