...
public abstract class ACassandraAccessor extends ALogger {
	...
	private String getPoolInfo() {
		return String.format("Pools size: A(%s)/P(%s)", _cassandraClientsActive
				.size(), _cassandraClientsPassive.size());
	}
	...
	public void setup() {
		try {
			...
			getLog()
					.debug(
							String.format("(host/port): (%s/%s)", getHost(),
									getPort()));
	...
	protected void finalize() throws Throwable {
		...
		if (getTransport() != null && getTransport().isOpen()) {
			getLog()
					.warn(
							"Cassandra trasport still in use, we should flush & close it!.");
			getTransport().flush();
			getTransport().close();
		}
		super.finalize();
